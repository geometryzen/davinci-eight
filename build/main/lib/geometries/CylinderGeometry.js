...
function computeWallVertices(height, radius, clockwise, stress, tilt, offset, angle, generator, heightSegments, thetaSegments, points, tangents, vertices, uvs) {
     ...
     *
     */
    var halfHeight = Vector3_1.Vector3.copy(height).scale(0.5);
    /**
     * A displacement in the direction of axis that we must move for each height step.
     */
    var stepH = Vector3_1.Vector3.copy(height).scale(1 / heightSegments);
    var iLength = heightSegments + 1;
    for (var i = 0; i < iLength; i++) {
        /**
         * The displacement to the current level.
         */
        var dispH = Vector3_1.Vector3.copy(stepH).scale(i).sub(halfHeight);
        var verticesRow = [];
        var uvsRow = [];
        /**
         ...
         */
        var arcPoints = arc3_1.arc3(radius, angle, generator, thetaSegments);
        /**
        ...
        for (var j = 0; j < jLength; j++) {
            ...
            var tangent = Spinor3_1.Spinor3.dual(point, false);
...
var CylinderSimplexPrimitivesBuilder = (function (_super) {
    ...
    function CylinderSimplexPrimitivesBuilder(height, cutLine, clockwise, mode) {
        ...
        _this.height = Vector3_1.Vector3.copy(height);
        _this.cutLine = Vector3_1.Vector3.copy(cutLine);
    ...
    CylinderSimplexPrimitivesBuilder.prototype.regenerate = function () {
        ...
        var generator = Spinor3_1.Spinor3.dual(Vector3_1.Vector3.copy(this.height).normalize(), false);
        var heightHalf = 1 / 2;
        var points = [];
        var tangents = [];
        ...
        if (!this.openWall) {
            for (var j = 0; j < thetaSegments; j++) {
                for (var i = 0; i < heightSegments; i++) {
                    ...
                    var n1 = Vector3_1.Vector3.dual(tangents[v1], true).normalize();
                    var n2 = Vector3_1.Vector3.dual(tangents[v2], true).normalize();
                    var n3 = Vector3_1.Vector3.dual(tangents[v3], true).normalize();
                    var n4 = Vector3_1.Vector3.dual(tangents[v4], true).normalize();
        ...
        if (!this.openCap) {
            ...
            var top_1 = Vector3_1.Vector3.copy(this.height).scale(heightHalf).add(this.offset);
            var tangent = Spinor3_1.Spinor3.dual(Vector3_1.Vector3.copy(this.height).normalize(), false).stress(this.stress).rotate(this.tilt);
            var normal = Vector3_1.Vector3.dual(tangent, true);
            points.push(top_1);
            for (var j = 0; j < thetaSegments; j++) {
                ...
                var uv2 = new Vector2_1.Vector2([(uv1.x + uv3.x) / 2, (uv1.y + uv3.y) / 2]);
                switch (this.mode) {
                    case GeometryMode_1.GeometryMode.MESH: {
        ...
        if (!this.openBase) {
            ...
            var bottom = Vector3_1.Vector3.copy(this.height).scale(-heightHalf).add(this.offset);
            var tangent = Spinor3_1.Spinor3.dual(Vector3_1.Vector3.copy(this.height).normalize(), false).neg().stress(this.stress).rotate(this.tilt);
            var normal = Vector3_1.Vector3.dual(tangent, true);
            points.push(bottom);
            for (var j = 0; j < thetaSegments; j++) {
                var v1 = vertices[0][j];
...
function getAxis(options) {
    ...
    else if (isDefined_1.isDefined(options.length)) {
        return R3_1.vec(0, mustBeNumber_1.mustBeNumber('length', options.length), 0);
    }
    else {
        return R3_1.vec(0, 1, 0);
    }
}
function getMeridian(options) {
    ...
    else if (isDefined_1.isDefined(options.radius)) {
        return R3_1.vec(0, 0, mustBeNumber_1.mustBeNumber('radius', options.radius));
    }
    else {
...
function cylinderPrimitive(options) {
    ...
    if (isDefined_1.isDefined(options.openWall)) {
        builder.openWall = mustBeBoolean_1.mustBeBoolean('openWall', options.openWall);
    }
    if (isDefined_1.isDefined(options.heightSegments)) {
        builder.flatSegments = mustBeInteger_1.mustBeInteger("heightSegments", options.heightSegments);
    }
    if (isDefined_1.isDefined(options.thetaSegments)) {
        builder.curvedSegments = mustBeInteger_1.mustBeInteger("thetaSegments", options.thetaSegments);
    }
    if (options.offset) {
...
function baseOptions(options) {
    ...
    var tilt = Geometric3_1.Geometric3.rotorFromDirections(canonicalAxis, axis);
    return { tilt: tilt };
}
/**
