...
var Vertex_1 = require("../atoms/Vertex");
var VectorN_1 = require("../math/VectorN");
function checkIntegerArg(name, n, min, max) {
    mustBeInteger_1.mustBeInteger(name, n);
    mustBeGE_1.mustBeGE(name, n, min);
    mustBeLE_1.mustBeLE(name, n, max);
    return n;
}
...
function lerp(a, b, alpha, data) {
    if (data === void 0) { data = []; }
    mustBeEQ_1.mustBeEQ('a.length', a.length, b.length);
    var dims = a.length;
    var i;
    var beta = 1 - alpha;
    for (i = 0; i < dims; i++) {
...
var Simplex = /** @class */ (function () {
    ...
    Simplex.boundaryMap = function (simplex) {
        ...
        var k = simplex.k;
        if (k === SimplexMode_1.SimplexMode.TRIANGLE) {
        ...
        else if (k === SimplexMode_1.SimplexMode.LINE) {
