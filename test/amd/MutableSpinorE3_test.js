define(['davinci-eight/math/MutableSpinorE3'], function(MutableSpinorE3)
{
  describe("MutableSpinorE3", function() {
    describe("constructor", function() {
      it("data argument should be preserved", function() {
        var data = [1, 2, 3, 4];
        var m = new MutableSpinorE3(data);
    ...
    describe("copy", function() {
      it("should preserved values and set modified flag", function() {
        var source = new MutableSpinorE3([1, 2, 3, 4]);
        var m = new MutableSpinorE3().copy(source);
      ...
      it("should preserved modified flag when no change", function() {
        var source = new MutableSpinorE3([0, 0, 0, 1]);
        var m = new MutableSpinorE3().copy(source);
        expect(m.data[0]).toBe(0);
        expect(m.data[1]).toBe(0);
        expect(m.data[2]).toBe(0);
    ...
    describe("exp", function() {
      it("should preserve the identity", function() {
        var m = new MutableSpinorE3([0, 0, 0, 1]);
        var r = m.exp();
        expect(m.data[0]).toBe(0);
        expect(m.data[1]).toBe(0);
        expect(m.data[2]).toBe(0);
      ...
      it("should correspond with scalar exponentiation", function() {
        var m = new MutableSpinorE3([0, 0, 0, 3]);
        var clone = m.clone();
        m.exp();
        expect(m.data[0]).toBe(0);
        expect(m.data[1]).toBe(0);
    ...
    describe("scale", function() {
      it("should multiply each coordinate by the scalar value", function() {
        var m = new MutableSpinorE3([2, 3, 5, 7]).scale(2);
