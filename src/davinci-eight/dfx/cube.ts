...
function cube(size: number = 1): Simplex[] {
  ...
  let front  = quad(vec0, vec1, vec2, vec3, attributes);
  let right  = quad(vec0, vec3, vec4, vec5, attributes);
  let top    = quad(vec0, vec5, vec6, vec1, attributes);
  let left   = quad(vec1, vec6, vec7, vec2, attributes);
  let bottom = quad(vec7, vec4, vec3, vec2, attributes);
  let back   = quad(vec4, vec7, vec6, vec5, attributes);
