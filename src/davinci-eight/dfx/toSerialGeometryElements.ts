...
function toSerialGeometryElements(simplices: Simplex[], geometryMeta?: GeometryMeta): SerialGeometryElements {
  ...
  var attributes: { [name: string]: SerialGeometryAttribute } = {};
  for (k = 0; k < keysLen; k++) {
    let output = outputs[k];
  ...
  return new SerialGeometryElements(geometryMeta.k, new VectorN<number>(indices, false, indices.length), attributes);
}
