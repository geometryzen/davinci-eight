...
let createGraphicsProgram = function(monitors: IContextMonitor[], vertexShader: string, fragmentShader: string, attribs: string[]): IGraphicsProgram {
    ...
    var self: IGraphicsProgram = {
        ...
        uniformMatrix2(name: string, transpose: boolean, matrix: Matrix2, canvasId: number = DEFAULT_CANVAS_ID) {
            mustBeString('name', name)
            mustBeInteger('canvasId', canvasId)
            let program = programsByCanvasId.getWeakRef(canvasId)
            if (program) {
        ...
        uniformMatrix3(name: string, transpose: boolean, matrix: Matrix3, canvasId: number = DEFAULT_CANVAS_ID) {
            mustBeString('name', name)
            mustBeInteger('canvasId', canvasId)
            let program = programsByCanvasId.getWeakRef(canvasId)
            if (program) {
        ...
        uniformMatrix4(name: string, transpose: boolean, matrix: Matrix4, canvasId: number = DEFAULT_CANVAS_ID) {
            mustBeString('name', name)
            mustBeInteger('canvasId', canvasId)
            let program = programsByCanvasId.getWeakRef(canvasId)
            if (program) {
